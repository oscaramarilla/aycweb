    ...
    <>
      <nav className={`fixed top-0 left-0 right-0 z-[100] transition-all duration-300 ${scrolled ? "bg-[#070810]/95 backdrop-blur-xl border-b border-white/[0.05] shadow-lg shadow-black/20" : "bg-transparent"}`}>
        <div className="max-w-6xl mx-auto px-6 h-20 flex items-center justify-between">
          ...
          {/* MOBILE TOGGLE */}
          <button className="md:hidden z-[110] p-2 relative w-10 h-10 flex items-center justify-center rounded-lg" onClick={toggleMenu} aria-label="Abrir Menú">
      ...
      <div className={`fixed inset-0 z-[90] bg-[#070810] transition-all duration-300 md:hidden ${isMobileMenuOpen ? "opacity-100 visible" : "opacity-0 invisible pointer-events-none"}`}>
        <div className="flex flex-col h-full overflow-y-auto px-6 pt-24 pb-24">
          ...
          <Link href="/soluciones" onClick={toggleMenu} className="rounded-2xl border border-violet-900/40 bg-violet-950/20 p-5 mb-6 active:scale-[0.98] transition-transform">
            <div className="flex items-center gap-3 mb-2">
      ...
      {/* Floating WhatsApp */}
      <a href={WA_CTA} target="_blank" rel="noopener noreferrer" className="md:hidden fixed bottom-4 right-4 z-[80] w-12 h-12 bg-[#25D366]/90 backdrop-blur-sm rounded-full flex items-center justify-center shadow-[0_4px_15px_rgba(37,211,102,0.3)] hover:bg-[#25D366] hover:scale-105 transition-all duration-200" aria-label="Contactar por WhatsApp">
