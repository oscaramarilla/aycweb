...
import { getInvertirDict, INVERTIR_I18N } from "@/lib/config/invertirParaguay";
...
export async function submitLeadParaguay(
  ...
  const locale =
    typeof rawLocale === "string" && rawLocale in INVERTIR_I18N ? rawLocale : "es";
  const dict = getInvertirDict(locale);
  ...
  if (!parsed.success) {
    return { ok: false, error: dict.form.errorInvalid };
  }

  const lead: LeadParaguay = parsed.data;
  const { score, tier } = scoreLead(lead);

  // Persistencia con service role (bypasea RLS; server-side únicamente).
  ...
  if (supabaseUrl && supabaseKey) {
    try {
      ...
      // Non-blocking: no rompemos el flujo del usuario si Supabase falla.
    }
  }

  // Resumen de WhatsApp para Oscar (siempre en español).
  const resumen = [
