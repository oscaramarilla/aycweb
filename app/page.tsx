...
// 🔥 Componente inyectado para hacer la magia de "Copiar al portapapeles"
const CampoCopia = ({ etiqueta, valor }: { etiqueta: string; valor: string }) => {
  ...
  const ejecutarCopia = () => {
    ...
    setCopiado(true);
    setTimeout(() => setCopiado(false), 2000); // Vuelve a la normalidad en 2 seg
  };

  return (
    <div className="flex items-center justify-between gap-2 p-2 mt-2 bg-zinc-900 rounded-lg border border-zinc-700 text-sm">
      <div className="overflow-hidden">
        <span className="text-zinc-400 mr-2">{etiqueta}:</span>
        <span className="text-white font-mono">{valor}</span>
      </div>
      <button 
        onClick={ejecutarCopia} 
        className={`flex-shrink-0 text-xs font-bold px-3 py-1.5 rounded transition ${copiado ? "bg-emerald-600 text-white" : "bg-blue-600 hover:bg-blue-500 text-white"}`}
      >
            ...
            <div className="p-6 text-center">
              {metodoPago === "fiat" ? (
                <div className="animate-in fade-in duration-300">
                  ...
                  {/* 🔥 DATOS REALES FIAT CON BOTONES DE COPIA */}
                  <div className="text-left bg-zinc-950 p-4 rounded-xl border border-zinc-800 text-sm space-y-2 mb-8 mx-auto max-w-sm">
                    <p><span className="text-zinc-500 font-bold">Banco:</span> Itaú</p>
                    <p><span className="text-zinc-500 font-bold">Titular:</span> Oscar Emigdio Amarilla Caceres</p>
                ...
                <div className="animate-in fade-in duration-300">
                  ...
                  {/* 🔥 DATOS REALES CRYPTO CON BOTÓN DE COPIA */}
                  <div className="text-left bg-blue-950/20 p-4 rounded-xl border border-blue-900/30 text-sm mb-8 mx-auto max-w-sm overflow-hidden">
                    <p className="text-zinc-500 font-bold mb-2">Billetera USDT (TRC20 Web3 Wallet):</p>
