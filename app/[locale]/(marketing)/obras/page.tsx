...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  const t = await getTranslations({ locale, namespace: "works.meta" });
  return {
    title: t("title"),
...
// IDs de YouTube fijos; títulos y tags vienen de messages.
const SECONDARY_VIDEO_IDS = ["vOCqg_zctec", "nuOeyANqlH8", "JBvZd0uXbzQ", "hcZL2UUKGsU"] as const;
...
export default async function ObrasPage({ params }: Props) {
  ...
  />
            ...
            <div className="mt-6 text-center px-2">
              <h3 className="text-lg font-bold text-white mb-2">{t("mainTitle")}</h3>
              <p className="text-sm text-slate-400 leading-relaxed">{t("mainDesc")}</p>
            </div>
          </div>
        </div>
      </section>
      ...
      <section className="relative z-10 px-6 mb-20">
        <div className="max-w-7xl mx-auto">
          <div className="text-center mb-10">
            <h2 className="text-2xl font-bold text-white">{t("moreTitle")}</h2>
            <div className="w-12 h-1 bg-blue-500/50 mx-auto mt-4 rounded-full"></div>
          </div>

          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 md:gap-8">
            {SECONDARY_VIDEO_IDS.map((id, index) => (
              <div key={id} className="flex flex-col items-center">
                <div className="w-full aspect-[9/16] bg-slate-900/50 rounded-[1.5rem] p-3 border border-slate-800 shadow-xl hover:border-blue-500/40 transition-colors duration-300 relative group">
                  ...
                  <iframe
                    ...
                    src={`https://www.youtube.com/embed/${id}?autoplay=0&loop=0&controls=1&rel=0`}
                    title={t(`v${index + 1}Title`)}
                    frameBorder="0"
                    allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture"
                    allowFullScreen
                    loading="lazy"
                  ...
                  <div className="mt-4 text-center pb-1">
                    <h4 className="text-[13px] font-bold text-slate-200 group-hover:text-blue-400 transition-colors">{t(`v${index + 1}Title`)}</h4>
                  </div>
                </div>
              </div>
            ))}
